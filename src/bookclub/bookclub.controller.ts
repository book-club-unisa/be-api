import { Controller, Delete, Headers, HttpException, HttpStatus, Inject, Param, Post} from '@nestjs/common';
...
export class BookclubController {
    ...
    async createBookclub(
        ...
        @Param('name') bookclubName : string){
            ...
            if(result != 'UNAUTHORIZED') return this.BookclubService.createBookclub(isbn,bookclubName,result);
            else throw new HttpException('NOT AUTHORIZED', HttpStatus.UNAUTHORIZED);
        }

        @Delete('/deleteBookclub/:name')
        async deleteBookclub(@Param('name') bookclubName : string, 
        @Headers('Authorization') token: string| undefined){
            const result = await this.loadPermissionsByToken(token);
            if(result != 'UNAUTHORIZED') return this.BookclubService.deleteBookclub(bookclubName,result);
            else throw new HttpException('NOT AUTHORIZED', HttpStatus.UNAUTHORIZED);
        }
    ...
    getAllBookclubs(): Promise<Bookclub[]>{
